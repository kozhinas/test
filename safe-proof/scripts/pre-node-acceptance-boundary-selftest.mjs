...
if (clean.status !== 0 || !(clean.stdout ?? '').includes('phase-3-4.safe-entrypoint selftest ok')) {
  throw new Error(
...
console.info(
  'pre-node-acceptance-boundary.selftest ok shell-syntax clean-bootstrap ambient-install+proxy-env=scrubbed node-options+node-path=pre-node-reject tls-trust-env=pre-node-reject',
);
