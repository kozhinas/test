...
if (process.argv[1] && path.resolve(process.argv[1]) === modulePath) {
  ...
  console.info(
    'phase-3-4.npm-install-env selftest ok ambient-npm-config=scrubbed ambient-node-env=scrubbed ambient-phase34-expo=scrubbed ambient-proxy=scrubbed case-insensitive node-runtime+tls-trust-injection=forbidden userconfig=pinned-empty globalconfig=pinned-empty distinct-config-files project-npmrc=forbidden',
  );
}
