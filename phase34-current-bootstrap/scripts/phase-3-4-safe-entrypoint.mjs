...
if (command === 'selftest') {
  ...
  for (const key of Object.keys(env)) {
    ...
    if (normalized.startsWith('git_')) {
      throw new Error(`phase34 safe entrypoint selftest: Git repository override environment survived (${key})`);
    }
    if (
  ...
  console.info(
    'phase-3-4.safe-entrypoint selftest ok pre-node-bootstrap=active child-env=sanitized git-overrides=scrubbed',
  );
  process.exit(0);
}
