...
const composeAttemptMarkerPath = path.join(
  root,
  'infra',
  'runtime-secrets',
  '.compose-attempted',
...
// A preflight or secret-staging failure must not tear down an existing healthy deployment or
// erase its evidence. The compose wrapper writes a deployment-bound marker immediately before
// it actually invokes `docker compose`; only that marker (or a successful up return) authorizes
// destructive rollback of the attempted deployment.
...
try {
  ...
  // The public runtime has now been replaced/updated, so any previous evidence is stale even
  // before the remaining live checks finish.
  ...
  if (!composeAttempted) {
    try {
      ...
      // Marker state is part of the destructive-rollback decision. If it cannot be read, the
      // provisioning state is unknown, so fail closed and attempt teardown while preserving the
      // marker-read failure alongside the original error.
      composeAttempted = true;
      rollbackErrors.push(
        new Error('phase34 deployment rollback: unable to read Compose attempt marker', {
          cause: markerError,
        }),
      );
    }
...
async function readComposeAttemptMarker() {
  try {
    return (await readFile(composeAttemptMarkerPath, 'utf8')).trim();
  } catch (error) {
...
async function assertCurrentComposeAttemptMarker() {
  const marker = await readComposeAttemptMarker();
  if (marker !== deploymentId) {
    throw new Error(
      'phase34 deployment: Compose up returned without the expected deployment-bound attempt marker',
    );
  }
}
